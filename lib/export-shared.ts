...
export async function exportSharedItemToPDF(
  ...
  // Payment Schedule - only show paid periods
  if (item.payment_schedule && item.payment_schedule.length > 0) {
    // Filter to only show periods that have been paid
    const paidPeriods = item.payment_schedule.filter(period => {
      const periodPayments = payments.filter(p => p.period_id === period.month)
      return periodPayments.length > 0
    })

    if (paidPeriods.length > 0) {
      htmlContent += `
      <h2 style="font-size: 16px; margin: 20px 0 10px 0;">Payment Schedule (Paid Periods)</h2>
      <table style="width: 100%; border-collapse: collapse; font-size: 10px; margin-bottom: 20px;">
        ...
        return `
              <tr style="background: ${bgColor};">
                ...
                <td style="padding: 6px; text-align: right; border: 1px solid #ddd; color: #22c55e; font-weight: bold;">${totalPaid.toLocaleString('en-US', { minimumFractionDigits: 2 })} THB</td>
              </tr>
            `
      }).join('')}
        </tbody>
